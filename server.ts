...
import { UserProfile, MatchProgress, Message, Subscription, UserReport } from "./src/types.js";
...
let usersDb: (UserProfile & { email?: string })[] = [
  {
    user_id: 'user_alice',
...
// Current logged-in member simulated state: Starts empty so they are a guest initially!
...
function calculateBaseCompatibility(userA: UserProfile, userB: UserProfile): number {
  ...
  if (userA.relationship_goal === userB.relationship_goal) {
    ...
    (userA.relationship_goal.includes('Long-term') || userA.relationship_goal.includes('Marriage')) &&
    (userB.relationship_goal.includes('Long-term') || userB.relationship_goal.includes('Marriage'))
  ) {
    score += 10;
  }
...
// 2. Restart/Reset Session (helpful for testing)
...
// In-Memory map to store verification codes (OTPs)
...
app.post("/api/auth/login", (req, res) => {
  ...
  // Cross-check password. If no password exists on pre-seeded profiles, check against 'password123'
...
// 3.4 User Registration (Multi-step - now persist password and prevent email collision)
app.post("/api/register", (req, res) => {
  ...
  if (existingUser) {
    if (existingUser.profile_completion < 20) {
      existingUser.full_name = full_name || existingUser.full_name;
      existingUser.age = Number(age) || existingUser.age;
      existingUser.gender = (gender as any) || existingUser.gender;
      existingUser.location = location || existingUser.location;
      existingUser.relationship_goal = (relationship_goal as any) || existingUser.relationship_goal;
      existingUser.bio = `Hey, I am ${full_name || existingUser.full_name}. New to SoulMatch but happy to be here. Reaching out with interests in ${interests ? interests.join(", ") : 'meaningful chats'}.`;
      existingUser.interests = interests || existingUser.interests;
      existingUser.profile_completion = 50;
      if (primary_photo) {
        existingUser.photo_url = primary_photo;
        existingUser.photos = [primary_photo];
      }
      currentUserSession = existingUser;
      
      currentSubscription = {
        subscription_id: 'sub_' + existingUser.user_id,
  ...
  const newUser: UserProfile = {
    ...
    age: Number(age) || 27,
    gender: (gender as any) || 'Non-binary',
    location: location || 'San Francisco, CA',
    religion: 'Spiritual',
    education: 'Undergraduate Degree',
    occupation: 'Creative Specialist',
    relationship_goal: (relationship_goal as any) || 'Long-term partnership',
...
Make the tone warm, welcoming, deeply human, smart, and free from typical AI cliché sentence structures (like 'dynamic companion', 'looking for adventure', 'fluent in sarcasm'). Offer a beautiful hook about their interests. Output only the bio prose directly, no extra introductory words.`;
...
// 9. Match Swiping Actions (Like / Pass)
app.post("/api/match/action", (req, res) => {
  const { targetId, action } = req.body; // 'like' or 'pass'
  
  // Check if there is an existing swipe
  ...
  // Simulate standard mutual matching rules:
  // Since key simulation allows all active Likes to match, we trigger a "Mutual Match" on Like!
  if (action === 'like') {
    status = 'matched';
    
    // Auto insert an icebreaker message in db to make it instantly alive!
    messagesDb.push({
...
// 10. AI Advisor (Get Detailed Gemini Match Compatibility Explanation)
...
// 11. Core messaging: Send Chat messages
app.post("/api/chat/send", (req, res) => {
  ...
  // Auto Reply after 3 seconds to emulate dynamic active users
  setTimeout(() => {
    const replyingUser = usersDb.find(u => u.user_id === receiverId);
    const replyText = text ? `That sounds wonderful! Tell me more about that. 😄` : `Thanks for sending that over! Really neat!`;
    messagesDb.push({
...
Avoid generic openers like 'How is your Sunday going?'. Craft something smart that references their shared interests, for example design, board games, or records. Output only the prompt message directly, no quotations, no header words.`;
  ...
  if (ai) {
    try {
      ...
      res.json({ success: true, icebreaker: `Hey ${matchUser.full_name}! I saw we both love graphic creative patterns and music records. What is your go-to espresso order or record to spin on a cozy Sunday morning? ☕💿` });
    }
  } else {
    res.json({
      success: true,
      icebreaker: `Hey ${matchUser.full_name}! I noticed we both share a quiet passion for craft aesthetics and hiking paths. If we could pack a backpack for any national park tomorrow, where would we head first? 🗺️🎒`
    });
  }
});

// 13. Safety Monitor: Check Chat safety (Gemini scanning for scam/harassment risks)
app.post("/api/chat/scan-safety", async (req, res) => {
  const { matchUserId } = req.body;
  
  // Filter messages between current user and match
  const conversation = messagesDb.filter(m => 
    (m.sender_id === 'current_user' && m.receiver_id === matchUserId) ||
    (m.sender_id === matchUserId && m.receiver_id === 'current_user')
  );
...
app.post("/api/subscribe", (req, res) => {
  ...
  currentSubscription = {
    ...
    user_id: 'current_user',
    plan: plan || 'premium',
    expiry_date: new Date(Date.now() + 3600000 * 24 * 365).toLocaleDateString()
  };
  res.json({ success: true, subscription: currentSubscription });
});

// 15. Block or Report users
...
app.get("/api/admin/stats", (req, res) => {
  ...
  res.json({
    totalUsers: totalUsersCount + 42, // pad some mock values for realism
...
// ==== GOOGLE DRIVE DATABASE BACKUP & SYNC MODULE ====
...
// GET status and files inside the specified Google Drive Folder
app.get("/api/drive/status", async (req, res) => {
  const token = (req.query.token as string) || googleDriveConfig.accessToken;
  const folderId = (req.query.folderId as string) || googleDriveConfig.folderId;
...
// Update Drive credentials and folder
...
// Push local server memory state to specified Google Drive folder
app.post("/api/drive/sync-push", async (req, res) => {
  ...
  try {
    ...
    // 1. Search for existing 'soulmatch_db_backup.json' file
    ...
    if (fileId) {
      ...
      // OVERWRITE content of the newly created file resource
      writeResponse = await fetch(`https://www.googleapis.com/upload/drive/v3/files/${fileId}?uploadType=media`, {
        method: "PATCH",
        headers: {
          "Authorization": `Bearer ${token}`,
    ...
    // Write individual user profile files inside folder to fulfill "write any folder/file inside this for each user data"
    ...
    googleDriveConfig.lastSyncTime = new Date().toISOString();
    googleDriveConfig.syncStatus = "connected";
    googleDriveConfig.lastError = null;
...
// Pull backup from Google Drive file and merge with local RAM databases
app.post("/api/drive/sync-pull", async (req, res) => {
  ...
  try {
    ...
    if (!targetFileId) {
      // Find 'soulmatch_db_backup.json' in this folder
      const listUrl = `https://www.googleapis.com/drive/v3/files?q='${folderId}'+in+parents+and+name='soulmatch_db_backup.json'+and+trashed=false&fields=files(id,name)`;
      const searchResponse = await fetch(listUrl, {
        headers: { "Authorization": `Bearer ${token}` }
      });
...
app.get("/api/auth/google/url", (req, res) => {
  ...
  // Dynamic redirect path back to callback route
...
// OAuth Callback exchange route handler
